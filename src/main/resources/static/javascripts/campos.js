...
window.onload = async function () {
  try {
    ...
    for (let campo of campos) {
      html += `<li class="list-group-item"><h5>${campo.nome}</h5><button onclick="toCampo(${campo.campoId})" class="btn btn-success">
            Visitar
        </button></li>`;
    }
...
function toCampo(campoId) {
  sessionStorage.setItem("campoId", campoId);
