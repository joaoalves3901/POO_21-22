...
window.onload = async function () {
  ...
  <h6><b>Telefone:</b> ${centro_saude.tlm}</h6>
  <h6><b>Morada:</b> ${centro_saude.morada}</h6><br>
  </div>`;
    ...
    // console.log(centro_saude.lat, centro_saude.lng)
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(async position => {
            ...
            map = L.mapquest.map("map", {
                ...
                for (let centro_saude of centros) {
                    L.marker([centro_saude.lat, centro_saude.long], {
